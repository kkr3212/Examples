...
namespace RPGGame.GameClient
{
    public partial class FormLogin : Form
    {
        ...
        {
            NetworkAPI.SetAuthServer(_tbServerIp.Text, _tbPortNo.Text.ToInt32());


            //  접속 가능한 World 목록 요청
            _lvWorld.Items.Clear();
            NetworkAPI.Auth_WorldList((response) =>
            {
        ...
        {
            //  UserToken으로 가입 요청
            NetworkAPI.Auth_RegisterGuest(_tbUserToken.Text, (response) =>
            {
        ...
        private void OnClick_Login(object sender, EventArgs e)
        {
            ...
            NetworkAPI.Auth_LoginGuest(worldId, UserToken, (response) =>
            {
                if (response.ResultCodeNo == ResultCode.Ok)
